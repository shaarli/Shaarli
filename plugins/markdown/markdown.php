...
/**
 * Un-escape content inside <code> HTML tags in given string.
 *
 * @param string $description input description text.
 *
 * @return string given string with un-escaped code blocks.
 */
function unescape_code_blocks($description)
{
    return preg_replace_callback(
        '#(?=< *code[^>]*>)(.*)?(?=</ *code[^>]*>)#is',
        function($matches) { return unescape($matches[1]); },
        $description
    );
}
...
function process_markdown($description)
{
    ...
    $processedDescription = unescape_code_blocks($processedDescription);
